...
// define this to enable debug output on the serial port at 500k/s (slows things down!)
//#define SERIAL_DEBUG

typedef struct _handler {
	void (*Init)();
	int (*Read)(unsigned char page, unsigned int address);		// note: Return is an int so that -1 can mean 'do not write to console'
	void (*Write)(unsigned char page, unsigned int address, unsigned char data);
} HANDLERS;
