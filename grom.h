...
// for the actual pages in question... flash page needs to use the same values!
// note that flash pages on this device are 512 bytes, so it needs to be aligned!
// GROMSTART assumes the total size of flash is 128k. These values are important
// both for the flash module and for the GROM module to know where the 64k address
// page split is. However, we have to be aware of the 16-bit limit which gives
// a maximum integer of 65535. So each page is 8k (15 pages is 120k)
#define GROMPAGES	15
#define GROMSTART	(0)

extern HANDLERS GromHandlers;
